  ...
  try
    ...
    case west_protocol_handler:handle_event(Cmd, DecMsg, S#state.server) of
      {ok, Res} ->
        ...
        {reply, {binary, BinRes}, Req, S#state{nb_bins = M + 1}};
      {error, Err} ->
    ...
    _:_ ->
      ...
      {reply, {binary, BinErr2}, Req, S#state{nb_bins = M + 1}}
  end;
websocket_handle({text, Msg}, Req, #state{nb_texts = N} = S) ->
  ?LOG_INFO("Received text msg (N=~p): ~p bytes~n", [N, byte_size(Msg)]),
  {reply, {text, <<"bad_encoding">>}, Req, S};
websocket_handle(_Data, Req, S) ->
  {ok, Req, S}.

websocket_info({event, Msg}, Req, S) ->
  {reply, {binary, Msg}, Req, S};
